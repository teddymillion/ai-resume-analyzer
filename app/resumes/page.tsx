...
function ResumesContent() {
  ...
  const { deleteResume, deletingId, error: deleteError } = useDeleteResume({
    onSuccess: (id) => {
      ...
      setAnalyses((prev) => prev.filter((a) => a.resume_id !== id))
    },
  })
  ...
  const analysisByResume = useMemo(() => {
    ...
    analyses.forEach((row) => map.set(row.resume_id, row))
    return map
  }, [analyses])
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-white/60 mt-2">Access your uploaded resumes and AI feedback in one place.</p>
          </div>
          <div className="flex flex-col items-end gap-2">
            {user && <span className="text-sm text-white/70">{user.email}</span>}
            <div className="flex gap-2">
              <Button variant="outline" className="border-white/15 bg-white/5 text-white hover:bg-white/10" onClick={() => router.push('/')}>
                Back to Analyzer
              </Button>
        ...
        {!loading && !error && resumes.length === 0 && (
          <div className="mt-10 rounded-2xl border border-white/10 bg-white/5 p-10 text-center">
            ...
            <p className="mt-2 text-sm text-white/60">Upload a resume to see analysis history here.</p>
            <Button className="mt-6 bg-cyan-500 text-slate-950 hover:bg-cyan-400" onClick={() => router.push('/')}>
              Upload Resume
            </Button>
          </div>
        )}
        ...
        {!loading && resumes.length > 0 && (
          <div className="mt-10 grid gap-6 md:grid-cols-2">
            {resumes.map((resume) => {
              ...
              return (
                <div key={resume.id} className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-xl">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-xs text-white/50">File</p>
                      <h3 className="text-lg font-semibold text-white mt-1">{resume.file_name}</h3>
                    </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-4">
                    <div className="rounded-xl border border-white/10 bg-white/5 p-3 text-center">
                      ...
                      <p className="text-2xl font-semibold text-cyan-300">{analysis?.overall_score ?? '—'}</p>
                    </div>
                    <div className="rounded-xl border border-white/10 bg-white/5 p-3 text-center">
                      <p className="text-xs text-white/50">ATS</p>
                      <p className="text-2xl font-semibold text-emerald-300">{analysis?.ats_score ?? '—'}</p>
                    </div>
                  </div>

                  <div className="mt-5 flex flex-wrap justify-end gap-2">
                    <Button variant="outline" className="border-white/15 bg-white/5 text-white hover:bg-white/10" onClick={() => router.push(`/resumes/${resume.id}`)}>
                      View Details
                    </Button>
                    <Button
                      variant="outline"
